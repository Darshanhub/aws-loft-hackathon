...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewViewProvider('prChatView', provider),
    vscode.commands.registerCommand('prChat.open', () => provider.reveal())
  );
}
...
class ChatViewProvider implements vscode.WebviewViewProvider {
    ...
    webviewView.webview.onDidReceiveMessage(async (msg) => {
      if (msg.type === 'openSettings') {
        await vscode.commands.executeCommand(
          ...
          '@ext:pr-chat-coderabbit PR Chat'
        );
      }
    });
  }
  ...
  private _getHtml(webview: vscode.Webview): string {
    ...
    const base = cfg.get<string>('serverBaseUrl') || 'http://127.0.0.1:8000';
    const owner = cfg.get<string>('owner') || 'org';
    const repo = cfg.get<string>('repo') || 'service';
    const prNumber = cfg.get<number>('prNumber') || 1;
    const author = cfg.get<string>('author') || 'vscode-user';
